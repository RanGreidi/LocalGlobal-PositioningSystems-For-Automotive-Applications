classdef Infrastructure < matlab.System & handle & matlab.system.mixin.Propagates & matlab.system.mixin.CustomIcon
    ...
    methods(Access = protected)
        ...
        function StopCommands = stepImpl(obj, V2Idata)
            ...
            % write here 
            %take map.vehicles and make a distance matrix 
            every_car_GPS_in_the_time_sample = []; 
            time_vec = [];
            every_car_position_in_the_time_sample = [];
            for i = 1:length(obj.map.Vehicles)
                time_vec = cat(1,time_vec, time);
                %every_car_position_in_the_time_sample = [every_car_position_in_the_time_sample obj.map.Vehicles(1,i).dynamics.position ]; 
                every_car_position_in_the_time_sample = cat(1 ,every_car_position_in_the_time_sample, obj.map.Vehicles(1,i).dynamics.position);
                every_car_GPS_in_the_time_sample = cat(1 ,every_car_GPS_in_the_time_sample, obj.map.Vehicles(1,i).dynamics.GPS);
            end
            every_car_position_in_the_time_sample = cat(2, every_car_position_in_the_time_sample, time_vec);

            % write to vehicale instance the v2v distances
            for i = 1:length(obj.map.Vehicles)
                obj.map.Vehicles(1,i).V2V_dist.all_vehicles_position = every_car_position_in_the_time_sample; 
